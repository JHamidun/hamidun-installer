...
# MCP. Скрипт лежит в паке и умеет всё это сам, но зовут его только из config.ps1 — а
# config идёт РАНЬШЕ нас (pydeps needs=["config"]), и Python на тот момент ещё не
# установлен. Там он честно печатал «после установки Python запусти вручную» и на этом
# всё заканчивалось: перезапустить его было некому, и на чистой машине доводка не
# выполнялась НИ РАЗУ. Ровно те полчаса ручной возни, ради устранения которых он писался.
#
# Здесь Python уже есть — тот самый $py. Скрипт идемпотентен: повторный прогон занимает
# секунды и ничего не ломает, поэтому двойной вызов (из config, если Python там всё же
# нашёлся, и отсюда) безвреден.
$runtimeScript = Join-Path $env:USERPROFILE '.claude\scripts\setup_runtime.py'
if (Test-Path $runtimeScript) {
    Write-Host "Довожу рантайм (браузер Playwright, маркетплейсы плагинов, зависимости)..."
    & $py $runtimeScript
    if ($LASTEXITCODE -ne 0) {
        Write-Host "  ВНИМАНИЕ: часть доводки не прошла. На работу конфига это не влияет —"
        Write-Host "  затронуты только соответствующие навыки. Повторить: python `"$runtimeScript`""
    }
} else {
    Write-Host "  (пропускаю доводку рантайма: $runtimeScript не найден — конфиг ещё не разложен)"
}
