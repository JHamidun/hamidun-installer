...
if ! /usr/bin/ditto -x -k "$ZIP" "$TARGET" </dev/null 2>/dev/null; then
  ...
  if ! /usr/bin/unzip -o -q "$ZIP" -d "$TARGET" </dev/null; then
    # Обе распаковки не смогли — это почти всегда место на диске или права на папку.
    # Раньше человек видел одну строку «Не удалось распаковать курс» и не знал, что
    # делать; при этом архив ЦЕЛЫЙ (SHA-256 сверен выше), так что дело не в файле.
    echo "Не удалось распаковать курс в папку $TARGET."
    CFREE_KB="$(df -k "$TARGET" 2>/dev/null | awk 'NR==2{print $4}')"
    [ -n "${CFREE_KB:-}" ] && echo "  Свободно на диске: $(( CFREE_KB / 1024 )) МБ."
    echo "  Архив курса при этом целый — проверка подлинности прошла."
    echo "  Что делать: 1) освободи 1-2 ГБ на диске; 2) убедись, что папка"
    echo "  $TARGET не открыта в другой программе; 3) нажми «Повторить»."
    exit 1
  fi
fi
if [ ! -f "$COURSE_DIR/CLAUDE.md" ]; then
