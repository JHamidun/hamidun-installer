...
verify_desktop_app() {
  ...
  local req="anchor apple generic and identifier \"$OPENAI_BUNDLE_ID\" and certificate leaf[subject.OU] = \"$OPENAI_TEAM_ID\""
  if ! "$CODESIGN" --verify --deep --strict -R "=$req" "$app" >/dev/null 2>&1; then
    echo "БЕЗОПАСНОСТЬ: .app не удовлетворяет designated requirement (подпись невалидна ИЛИ не тот издатель/Team $OPENAI_TEAM_ID/bundle $OPENAI_BUNDLE_ID): $app. Пропускаю (fail-closed)."; return 1
  fi
