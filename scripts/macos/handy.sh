...
if [ -z "$DMG" ]; then
  ...
  # Но «в наборе нет вообще» и «в наборе есть только под другой Mac» — РАЗНЫЕ вещи,
  # и человеку нужно знать, какая из них про него (Handy раздаётся двумя отдельными
  # dmg, universal-сборки upstream не делает).
  if ! hm_arch_note_missing "${HM_VENDOR:-}/apps" "handy-macos-" ".dmg" "Handy (голосовой ввод)"; then
