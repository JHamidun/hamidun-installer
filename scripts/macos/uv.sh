...
#
# В skip-ветке разделяем два РАЗНЫХ случая: (а) компонента нет в сборке вообще
# (lite-издание); (б) в сборке лежит версия только под другую архитектуру Mac.
# Раньше оба выглядели одинаково, и человек не понимал, почему у соседа встало.
# Обе ветки только ПЕЧАТАЮТ текст: ни сети, ни подстановки чужого источника —
# bundled-only остаётся bundled-only (hm_arch_note_missing ничего не качает).
UV_TGZ="${HM_VENDOR:-}/apps/uv-macos-$(arch_tag).tar.gz"
if [ -z "${HM_VENDOR:-}" ] || [ ! -f "$UV_TGZ" ]; then
  hm_arch_note_missing "${HM_VENDOR:-}/apps" "uv-macos-" ".tar.gz" "uv" \
    || echo "uv не вошёл в эту сборку — пропускаю. Всё остальное работает без него."
  exit 120
fi
