...
if __name__ == "__main__":
    # --selftest: проверка «агент вообще работоспособен» для установщика.
    # БЕЗ побочных эффектов: не берём порт-замок, не поднимаем серверы, не трогаем
    # системный прокси и не порождаем ssh-туннель.
    #
    # Раньше установщик проверял агента, запуская его по-настоящему на 3 секунды и
    # глядя, жив ли он. Это давало сразу три беды:
    #   • второй экземпляр штатно выходит с кодом 0 (порт-замок занят работающим
    #     мостом) — и это читалось как «агент сломан»;
    #   • чтобы замок освободить, установщик выгружал работающий мост, а --headless
    #     уважает сохранённое enabled — то есть проба поднимала НАСТОЯЩИЙ мост и
    #     оставляла осиротевший `ssh -D 1080` после kill (обработчик SIGTERM
    #     туннель не трогает), и новый агент потом бесконечно бился в занятый порт;
    #   • медленное завершение старого агента (снятие системного прокси на каждом
    #     сетевом сервисе) не укладывалось в паузу — рабочий мост объявлялся сломанным.
    # Отдельный режим убирает все три причины разом.
    if "--selftest" in sys.argv:
        try:
            load_cfg()          # конфиг читается и разбирается
            Bridge()            # объект строится, зависимости на месте
            print("HM-BRIDGE-SELFTEST-OK")
            sys.exit(0)
        except Exception as e:  # noqa: BLE001 — наружу нужен только вердикт
            print("HM-BRIDGE-SELFTEST-FAIL: %s" % e)
            sys.exit(1)
