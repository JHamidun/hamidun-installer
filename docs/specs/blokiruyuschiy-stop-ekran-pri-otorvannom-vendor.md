...
6. Нет проверки в main → достаточно любого пути, минующего кнопку (переоткрытое окно, гонка при загрузке, ошибка в `refreshDerived`), чтобы стартовал `msiexec`/`uv` в состоянии, где офлайн-архивов нет: половина стека встанет, половина нет, и человек останется с недоустановленной машиной без понятного отката.
...
- **Не чинит проблему сам.** Стоп-экран только останавливает и объясняет; снятие карантина, перемонтирование dmg и перезапуск — отдельная фича самолечения (`macos-samolechenie-karantina-i-translokacii`, `ipcMain.handle('mac-selfheal')` в `src/main.js:3429`, `bindMacSelfHeal` в `src/renderer/app.js:808-850`).
