...
5. **Вырез для vm привязан к строке комментария.** `src.indexOf('\n// Жёсткий стоп ДО установки')` внутри `cutDetectors` (`test/run-tests.js:8063`) — переписывание этого комментария в `src/main.js:136` сломает тесты либо (хуже) сдвинет границу выреза незаметно.
