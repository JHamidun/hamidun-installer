...
9. Маркер `offlineEdition` уехал в коммит — последствие зависит от ЗНАЧЕНИЯ, и два случая противоположны. `offlineEdition: true` → выпущенная сборка `dist:mac` показывает стоп-экран про карантин сразу после установки, и ни один компонент поставить нельзя; `assert:clean` этот случай не ловит — в `tools/assert-clean.js` есть проверки только на `edition === 'lite'` (`tools/assert-clean.js:73`) и `offlineEdition === false` (`tools/assert-clean.js:78-81`), на `=== true` нет ни одной. `offlineEdition: false` — тот случай, который в этом репозитории реально протекает (`tools/build-lite.js:320` пишет `cfg.offlineEdition = false` прямо в рабочее дерево, `tools/build-lite.js:327`; восстановление — в `finally`, `tools/build-lite.js:376-377`) — стоп-экрана НЕ даёт: `isOfflineEdition()` требует строго `=== true` (`src/main.js:90`). До сборки он и не доходит: `dist:mac` начинается с `assert:clean` (`package.json:20`), который на `false` прерывает запуск.
...
5. **Вырез для vm привязан к строке комментария.** `src.indexOf('\n// Жёсткий стоп ДО установки')` внутри `cutDetectors` (`test/run-tests.js:8063`) — переписывание этого комментария в `src/main.js:134` сломает тесты либо (хуже) сдвинет границу выреза незаметно.
