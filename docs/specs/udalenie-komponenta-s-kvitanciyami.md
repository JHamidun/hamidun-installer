...
4. Если бы правку settings.json пустили общим путём удаления, сломанный или чужой
   ...
   45832, где после удаления скрепки никто не слушает — четырнадцать хук-записей ×2,2 с
   таймаута дают около полуминуты мёртвого ожидания в КАЖДОЙ сессии Claude Code.
...
- Возврат hook-url — не удаление, а замена подстроки в `settings.json` на плейсхолдер
  пака; чужие записи и правки пользователя в файле не трогаются.
