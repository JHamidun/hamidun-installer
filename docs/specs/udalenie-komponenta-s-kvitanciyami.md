...
- `mascot` — сперва платформонезависимая цель `hookurl` на `~/.claude/settings.json`
  ...
  сначала стоит проверка существования (`src/main.js:3209`), и при отсутствующем
  ...
  отказ удалять .app», `src/main.js:3212`), а один `failed` роняет ВСЮ деинсталляцию
  маскота в `ok:false` с возвратом маркера (`src/main.js:3344-3352`).
...
(`src/main.js:372`) возвращает осиротевшие tombstone обратно в маркеры — иначе кнопка
...
- На macOS без доступного vendor-бандла `.app` маскота НЕ удаляется — по коду это
  ...
  (`src/main.js:3209`, `if (!fs.existsSync(t.path)) return { status: 'absent', … }`).
  Есть `~/Applications/<appName>.app` — цель отдаёт `failed` («ЗАЩИТА: нет эталонного
  CFBundleIdentifier», `src/main.js:3212`) и роняет всю деинсталляцию
  (`src/main.js:3344-3352`). Бандла там нет (перетащен в `/Applications`, снесён
...
- Возврат hook-url — не удаление, а замена подстроки в `settings.json`: механика
  подтверждена (`src/uninstall-targets.js:201-207` → `src/main.js:3165` →
