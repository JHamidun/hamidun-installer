  ...
  return await new Promise((resolve) => {
    ...
    const stallTimer = setInterval(() => {
      ...
      if (mins >= 10 && mins >= stallWarned + 10) {
        ...
        // «Шаг сам прервётся по своему лимиту» отсюда убрано: такого лимита НЕТ.
        // Дочерний процесс спавнится без timeout (watchdog намеренно без убийства —
        // kill посреди MSI-транзакции хуже висяка), и внутри скриптов долгие вызовы
        // тоже без ограничения: Start-Process -Wait у установщиков, npm install, pip,
        // распаковка. Единственный настоящий лимит — PT10M у де-элевированных задач
        // планировщика, а это не про этот шаг. Обещать человеку автоматическое
        // спасение, которого не существует, — то же самое, что советовать ждать вечно.
        const m = '[внимание] шаг «' + id + '» не выдаёт вывод ' + mins + ' мин. ' +
          ...
          'распаковку, идёт обновление Windows или машина медленная. Сам по себе шаг не ' +
          'прервётся — решение за тобой: если ничего не меняется больше часа, закрой окно ' +
          '(это остановит дочерние процессы) и запусти установщик заново — уже сделанное ' +
          'сохранится, повторится только незавершённое.';
        send(m); logLine(m);
      }
    }, 60000);
