...
ok('H1/M8: restore сверяется с СОДЕРЖИМЫМ снапшота (hash/cmp, не Test-Path); rm снапшота и «Вернул» только при успехе', () => {
  ...
  assert(/Get-Sha256Hex/.test(fnR), 'ps1: файл сверяется SHA-256 (.NET) со снапшотом — ШАБЛОН install.ps1 не сойдёт за ключи (M8)');
  assert(!/Get-FileHash/.test(ps), 'ps1: Get-FileHash НЕ используется (P1: недоступен под PS7-первым PSModulePath) — SHA через .NET');
...
// ---- P0-1/P0-2/P0-3/P1 (статически): 3 P0 + 1 P1 ложно-успешных пути clean/repair ----

console.log('== P0-1/P0-2/P0-3/P1 (статически): stale-conflict, атомарность снапшота, tg -wal/-shm, SHA без Get-FileHash ==');

// P0-3: clean-preserve список ТОЧНО совпадает с additive-исключениями (оба покрывают
// tg_session.session-wal/-shm — иначе clean теряет незачекпоинченную TG-авторизацию).
ok('P0-3: preserve-список clean == additive-исключения (ps1 множества идентичны; sh глобы покрывают), tg -wal/-shm включены', () => {
  const ps = fs.readFileSync(path.join(ROOT, 'scripts', 'windows', 'config.ps1'), 'utf8');
  const parsePsArray = (name) => {
    const m = ps.match(new RegExp('\\$' + name + ' = @\\(([\\s\\S]*?)\\)'));
    assert(m, 'ps1: $' + name + ' найден');
    return new Set((m[1].match(/'([^']+)'/g) || []).map((x) => x.replace(/'/g, '')));
  };
  const preserve = parsePsArray('preserveFiles');
  const exclude = parsePsArray('excludeNames');
  ['tg_session.session-wal', 'tg_session.session-shm'].forEach((n) => {
    assert(preserve.has(n), 'ps1 preserveFiles содержит ' + n);
    assert(exclude.has(n), 'ps1 excludeNames содержит ' + n);
  });
  assert(preserve.size === exclude.size, 'ps1: |preserve|==|exclude| (' + preserve.size + ' vs ' + exclude.size + ')');
  preserve.forEach((n) => assert(exclude.has(n), 'ps1: ' + n + ' в preserve, но НЕ в exclude'));
  exclude.forEach((n) => assert(preserve.has(n), 'ps1: ' + n + ' в exclude, но НЕ в preserve'));

  const sh = fs.readFileSync(path.join(ROOT, 'scripts', 'macos', 'config.sh'), 'utf8');
  const pf = sh.match(/^PRESERVE_FILES="([^"]+)"$/m);
  assert(pf, 'sh: PRESERVE_FILES найден');
  const preserveSh = pf[1].split(/\s+/).filter(Boolean);
  ['tg_session.session-wal', 'tg_session.session-shm'].forEach((n) =>
    assert(preserveSh.indexOf(n) !== -1, 'sh PRESERVE_FILES содержит ' + n));
  const iAdd = sh.indexOf('# === АДДИТИВНАЯ доустановка');
  const shBranch = sh.slice(iAdd, sh.indexOf('\nelse\n', iAdd));
  const excl = (shBranch.match(/--exclude='([^']+)'/g) || []).map((x) => x.replace(/--exclude='|'/g, ''));
  const covers = (name) => excl.some((e) => e.endsWith('*') ? name.startsWith(e.slice(0, -1)) : e === name);
  preserveSh.forEach((n) => assert(covers(n), 'sh: preserve-файл ' + n + ' НЕ покрыт additive-исключением'));
  const justified = (e) => e.endsWith('*')
    ? preserveSh.some((n) => n.startsWith(e.slice(0, -1)))
    : preserveSh.indexOf(e) !== -1;
  excl.filter((e) => !e.endsWith('/')).forEach((e) => assert(justified(e), 'sh: exclude ' + e + ' не соответствует ни одному preserve-файлу'));
});

// P0-1: stale-снапшот с конфликтом (живой отличается от снапшота) → abort ДО wipe.
ok('P0-1: stale-снапшот с конфликтом → abort ДО snapshot/wipe; сравнение по СОДЕРЖИМОМУ (SHA/cmp), не по существованию', () => {
  const ps = fs.readFileSync(path.join(ROOT, 'scripts', 'windows', 'config.ps1'), 'utf8');
  assert(/function Test-StaleSnapshotConflict/.test(ps), 'ps1: есть Test-StaleSnapshotConflict');
  const iConf = ps.indexOf('if (Test-StaleSnapshotConflict $preserveDir)');
  const iSnap = ps.indexOf('$snapOk = Snapshot-UserData $preserveDir');
  const iInst = ps.indexOf('& $installer -BackupExisting -SkipDeps');
  assert(iConf !== -1 && iSnap !== -1 && iInst !== -1 && iConf < iSnap && iConf < iInst, 'ps1: конфликт-проверка ДО снапшота и install');
  const guard = ps.slice(iConf, iConf + 800);
  assert(/exit 1/.test(guard) && /КОНФЛИКТУЕТ/.test(guard), 'ps1: конфликт → сообщение + exit 1 (fail-closed)');
  const fnConf = ps.slice(ps.indexOf('function Test-StaleSnapshotConflict'), ps.indexOf('function Snapshot-UserData'));
  assert(/Get-Sha256Hex/.test(fnConf), 'ps1: конфликт сверяется SHA (шаблон != реальные ключи)');
  assert(/return \$true/.test(fnConf), 'ps1: различие/несравнимость → конфликт $true');

  const sh = fs.readFileSync(path.join(ROOT, 'scripts', 'macos', 'config.sh'), 'utf8');
  assert(/stale_snapshot_conflict\(\)/.test(sh), 'sh: есть stale_snapshot_conflict');
  const iConfSh = sh.indexOf('if stale_snapshot_conflict "$PRESERVE_DIR"; then');
  const iSnapSh = sh.indexOf('if ! snapshot_user_data "$PRESERVE_DIR"; then');
  const iInstSh = sh.indexOf('bash "$CLONE/install.sh" --backup --skip-deps');
  assert(iConfSh !== -1 && iSnapSh !== -1 && iInstSh !== -1 && iConfSh < iSnapSh && iConfSh < iInstSh, 'sh: конфликт-проверка ДО снапшота и install');
  const guardSh = sh.slice(iConfSh, iConfSh + 800);
  assert(/exit 1/.test(guardSh) && /КОНФЛИКТУЕТ/.test(guardSh), 'sh: конфликт → сообщение + exit 1');
  const fnConfSh = sh.slice(sh.indexOf('stale_snapshot_conflict()'), sh.indexOf('snapshot_user_data()'));
  assert(/cmp -s "\$s" "\$l" \|\| return 0/.test(fnConfSh), 'sh: конфликт сверяется cmp по содержимому');
  assert(/\[ -f "\$l" \] \|\| return 0/.test(fnConfSh), 'sh: живой не обычный файл (каталог) → конфликт (fail-closed)');
});

// P0-2: снапшот атомарен — детект конкурентной модификации отпечатком/манифестом ДО и ПОСЛЕ.
ok('P0-2: снапшот детектирует конкурентную модификацию (отпечаток файла + манифест каталога ДО/ПОСЛЕ копии, до 3 попыток)', () => {
  const ps = fs.readFileSync(path.join(ROOT, 'scripts', 'windows', 'config.ps1'), 'utf8');
  const fnSnap = ps.slice(ps.indexOf('function Snapshot-UserData'), ps.indexOf('function Restore-UserData'));
  assert(/Get-FileFingerprint/.test(fnSnap), 'ps1: файл-снапшот сверяет отпечаток источника (размер+mtime)');
  assert(/\$fpAfter -and \(\$fpAfter -eq \$fpBefore\)/.test(fnSnap), 'ps1: источник не должен меняться во время копии файла');
  assert(/Get-DirManifest/.test(fnSnap), 'ps1: каталог-снапшот сверяет манифест дерева ДО/ПОСЛЕ');
  assert(/\$manAfter -eq \$manBefore/.test(fnSnap), 'ps1: манифест дерева не должен меняться во время копии');
  assert(/\$attempt -le 3/.test(fnSnap), 'ps1: до 3 попыток на консистентную копию');

  const sh = fs.readFileSync(path.join(ROOT, 'scripts', 'macos', 'config.sh'), 'utf8');
  const fnSnapSh = sh.slice(sh.indexOf('snapshot_user_data()'), sh.indexOf('restore_user_data()'));
  assert(/fp_before="\$\(hm_fsize "\$s"\):\$\(hm_fmtime "\$s"\)"/.test(fnSnapSh), 'sh: отпечаток источника ДО копии (размер:mtime)');
  assert(/\[ "\$fp_before" = "\$fp_after" \]/.test(fnSnapSh), 'sh: источник не менялся во время копии файла');
  assert(/cmp -s "\$s" "\$dst\/\$f"/.test(fnSnapSh), 'sh: копия побайтно сверяется с текущим источником');
  assert(/man_before="\$\(hm_dir_manifest "\$s"\)"/.test(fnSnapSh) && /\[ "\$man_before" = "\$man_after" \]/.test(fnSnapSh), 'sh: манифест дерева ДО/ПОСЛЕ копии каталога');
  assert(/for attempt in 1 2 3/.test(fnSnapSh), 'sh: до 3 попыток на консистентную копию');
});

// P1: SHA через .NET, Get-FileHash не используется (PS7-первый PSModulePath не ломает restore).
ok('P1: SHA-256 через .NET (Get-FileHash полностью убран — недоступен под PS7-первым PSModulePath)', () => {
  const ps = fs.readFileSync(path.join(ROOT, 'scripts', 'windows', 'config.ps1'), 'utf8');
  assert(/function Get-Sha256Hex/.test(ps), 'ps1: есть helper Get-Sha256Hex');
  assert(/\[System\.Security\.Cryptography\.SHA256\]::Create\(\)/.test(ps), 'ps1: SHA256 через .NET');
  assert(/\[System\.IO\.File\]::Open\(/.test(ps), 'ps1: чтение файла через .NET (FileShare.ReadWrite)');
  assert(!/Get-FileHash/.test(ps), 'ps1: Get-FileHash полностью убран');
});
...
if (bashAvailable()) {
  ...
  console.log('== P0-1/P0-3 config.sh CLEAN (функционально): stale-conflict abort, tg -wal/-shm restore ==');

  ok('P0-1 config.sh clean: stale-снапшот РЕАЛЬНЫЕ ключи + живой ШАБЛОН → конфликт, abort ДО wipe, реальные НЕ теряются, «Вернул» НЕ печатается', () => {
    const { base, home, clone } = mkFakeCleanHome();
    try {
      fs.writeFileSync(clone + '/install.sh', FAKE_INSTALL_SH_CLEAN);
      // stale-снапшот прерванного прогона: РЕАЛЬНЫЕ ключи
      fs.mkdirSync(home + '/.hamidun-setup/preserve', { recursive: true });
      fs.writeFileSync(home + '/.hamidun-setup/preserve/.credentials.master.env', 'KEY=REAL-SECRET');
      // живой файл — ШАБЛОН (install прошлого прогона успел положить его перед крахом)
      fs.writeFileSync(home + '/.claude/.credentials.master.env', 'KEY=TEMPLATE');
      const r = runConfigShClean(home, clone);
      assert(r.status !== 0, 'ненулевой выход (конфликт → abort): ' + (r.stdout || '') + (r.stderr || ''));
      assert(/КОНФЛИКТУЕТ/.test(r.stdout || ''), 'сообщение о конфликте stale-снапшота');
      assert(!fs.existsSync(home + '/.install-ran'), 'install.sh НЕ запускался (abort ДО wipe)');
      assert(!/Вернул твои ключи/.test(r.stdout || ''), '«Вернул…» НЕ печатается на шаблоне');
      assert.strictEqual(fs.readFileSync(home + '/.hamidun-setup/preserve/.credentials.master.env', 'utf8'), 'KEY=REAL-SECRET',
        'реальные ключи в снапшоте ЦЕЛЫ (не потеряны)');
      assert.strictEqual(fs.readFileSync(home + '/.claude/.credentials.master.env', 'utf8'), 'KEY=TEMPLATE',
        'живой файл не тронут (не вайпнут); реальные ключи не подменены под зелёный рапорт');
    } finally { try { fs.rmSync(base, { recursive: true, force: true }); } catch (e) { /* ignore */ } }
  });

  ok('P0-1 config.sh clean: stale-снапшот, живой файл ОТСУТСТВУЕТ (истинный краш между wipe и restore) → БЕЗ ложного конфликта, штатный clean', () => {
    const { base, home, clone } = mkFakeCleanHome();
    try {
      fs.writeFileSync(clone + '/install.sh', FAKE_INSTALL_SH_CLEAN);
      fs.mkdirSync(home + '/.hamidun-setup/preserve', { recursive: true });
      fs.writeFileSync(home + '/.hamidun-setup/preserve/.credentials.master.env', 'KEY=REAL-SECRET');
      fs.rmSync(home + '/.claude/.credentials.master.env', { force: true });
      const r = runConfigShClean(home, clone);
      assert(!/КОНФЛИКТУЕТ/.test(r.stdout || ''), 'нет ложного конфликта, когда живой файл отсутствует');
      assert.strictEqual(r.status, 0, 'exit 0 (штатный clean): ' + (r.stdout || '') + (r.stderr || ''));
    } finally { try { fs.rmSync(base, { recursive: true, force: true }); } catch (e) { /* ignore */ } }
  });

  ok('P0-3 config.sh clean: tg_session.session-wal/-shm сохранены и восстановлены поверх свежей базы', () => {
    const { base, home, clone } = mkFakeCleanHome();
    try {
      fs.writeFileSync(clone + '/install.sh', FAKE_INSTALL_SH_CLEAN);
      fs.writeFileSync(home + '/.claude/tg_session.session-wal', 'TG-WAL-UNCHECKPOINTED');
      fs.writeFileSync(home + '/.claude/tg_session.session-shm', 'TG-SHM');
      const r = runConfigShClean(home, clone);
      assert.strictEqual(r.status, 0, 'exit 0: ' + (r.stdout || '') + (r.stderr || ''));
      assert.strictEqual(fs.readFileSync(home + '/.claude/tg_session.session-wal', 'utf8'), 'TG-WAL-UNCHECKPOINTED', 'tg -wal восстановлен (clean==additive)');
      assert.strictEqual(fs.readFileSync(home + '/.claude/tg_session.session-shm', 'utf8'), 'TG-SHM', 'tg -shm восстановлен');
    } finally { try { fs.rmSync(base, { recursive: true, force: true }); } catch (e) { /* ignore */ } }
  });
} else {
...
if (powershellAvailable()) {
  ...
  console.log('== P0-1/P0-3/P1 config.ps1 CLEAN (функционально): stale-conflict abort, tg -wal/-shm restore, SHA без Get-FileHash ==');

  ok('P0-1 config.ps1 clean: stale-снапшот РЕАЛЬНЫЕ ключи + живой ШАБЛОН → конфликт, abort ДО wipe, реальные НЕ теряются, «Вернул» НЕ печатается', () => {
    const { base, home, clone } = mkFakeCleanHome();
    try {
      fs.writeFileSync(clone + '/install.ps1', FAKE_INSTALL_PS1_CLEAN);
      fs.mkdirSync(home + '/.hamidun-setup/preserve', { recursive: true });
      ...
      assert(!/Вернул твои ключи/.test(r.stdout || ''), '«Вернул…» НЕ печатается на шаблоне');
      assert.strictEqual(fs.readFileSync(home + '/.hamidun-setup/preserve/.credentials.master.env', 'utf8'), 'KEY=REAL-SECRET',
        'реальные ключи в снапшоте ЦЕЛЫ');
      assert.strictEqual(fs.readFileSync(home + '/.claude/.credentials.master.env', 'utf8'), 'KEY=TEMPLATE',
        'живой файл не тронут; реальные ключи не подменены под зелёный рапорт');
    } finally { try { fs.rmSync(base, { recursive: true, force: true }); } catch (e) { /* ignore */ } }
  });

  ok('P0-3 config.ps1 clean: tg_session.session-wal/-shm сохранены и восстановлены поверх свежей базы', () => {
    const { base, home, clone } = mkFakeCleanHome();
    try {
      fs.writeFileSync(clone + '/install.ps1', FAKE_INSTALL_PS1_CLEAN);
      fs.writeFileSync(home + '/.claude/tg_session.session-wal', 'TG-WAL-UNCHECKPOINTED');
  ...
  ok('P1 config.ps1 clean: PS7-первый PSModulePath (Get-FileHash мог бы не найтись) → restore на .NET SHA всё равно успешен', () => {
    const { base, home, clone } = mkFakeCleanHome();
    try {
      fs.writeFileSync(clone + '/install.ps1', FAKE_INSTALL_PS1_CLEAN);
      // проблемный env: модули PowerShell 7 идут ПЕРЕД 5.1 в PSModulePath (Get-FileHash-риск)
      const ps7first = 'C:\\Program Files\\PowerShell\\7\\Modules;' + (process.env.PSModulePath || '');
      const r = runConfigPs1Clean(home, clone, { PSModulePath: ps7first });
      assert.strictEqual(r.status, 0, 'exit 0 (restore на .NET SHA, без Get-FileHash): ' + (r.stdout || '') + (r.stderr || ''));
      assert.strictEqual(fs.readFileSync(home + '/.claude/.credentials.master.env', 'utf8'), 'KEY=NEW',
        'ключи восстановлены (SHA-сверка сработала без Get-FileHash)');
      assert(/Вернул твои ключи/.test(r.stdout || ''), 'честный рапорт о возврате');
    } finally { try { fs.rmSync(base, { recursive: true, force: true }); } catch (e) { /* ignore */ } }
  });
} else {
...
async function asyncTests() {
  ...
  // ---- P0-2 (функционально): подмена/занятость chats.db во время снапшота → детектируется ----
  // config обязан НЕ выдать зелёный рапорт на несогласованном chats.db: либо снапшот увидит
  // рассинхрон источника до/после копии (config.sh — непрерывный писатель + cmp), либо копия
  // вовсе не снимется (config.ps1 — эксклюзивная блокировка, как у работающего SQLite/Claude).
  const { spawn } = require('child_process');
  if (bashAvailable()) {
    console.log('== P0-2 config.sh CLEAN (функционально): непрерывная запись в chats.db → не зелёный ==');
    await okAsync('P0-2 config.sh clean: writer непрерывно меняет chats.db во время copy-фазы → снапшот несогласован → НЕ зелёный (abort/повтор)', async () => {
      const { base, home, clone } = mkFakeCleanHome();
      ...
      try {
        ...
        fs.writeFileSync(chats, 'X'.repeat(4194304));
        fs.writeFileSync(sentinel, '1');
        const w = 'end=$((SECONDS+30)); i=0; while [ -e "' + sentinel + '" ] && [ $SECONDS -lt $end ]; do ' +
          'i=$((i+1)); printf "%*s" $(((i%64+1)*65536)) "" > "' + chats + '"; done';
        writer = spawn('bash', ['-c', w], { detached: false, windowsHide: true, stdio: 'ignore' });
        // warm-up: убеждаемся, что писатель реально меняет размер chats.db
        let prev = -1, seen = 0, tries = 0;
        while (seen < 4 && tries < 800) {
          await new Promise((res) => setTimeout(res, 5));
          let sz = -1; try { sz = fs.statSync(chats).size; } catch (e) { /* ignore */ }
          if (sz !== prev) { seen++; prev = sz; }
          tries++;
        }
        assert(seen >= 4, 'фоновый писатель активен (размер chats.db меняется, seen=' + seen + ')');
        const r = runConfigShClean(home, clone);
        // ИНВАРИАНТ: config НЕ рапортует зелёный успех на несогласованном chats.db (снапшот
        // видит рассинхрон → abort до wipe; в редком гэпе — restore ловит cmp'ом → тоже не зелёный).
        assert(r.status !== 0, 'ненулевой выход (не зелёный): ' + (r.stdout || '') + (r.stderr || ''));
        assert(!/Вернул твои ключи/.test(r.stdout || ''), 'никакого ложного зелёного «Вернул» на несогласованном снапшоте');
        assert(fs.existsSync(chats), 'живой chats.db на месте (не потерян)');
      } finally {
  ...
  if (powershellAvailable()) {
    console.log('== P0-2 config.ps1 CLEAN (функционально): chats.db эксклюзивно занят (SQLite/Claude) → abort до wipe ==');
    await okAsync('P0-2 config.ps1 clean: chats.db эксклюзивно занят во время copy-фазы → консистентную копию не снять → abort ДО wipe, не зелёный', async () => {
      const { base, home, clone } = mkFakeCleanHome();
      const sentinel = base + '/writer-on';
      ...
      try {
        fs.writeFileSync(clone + '/install.ps1', FAKE_INSTALL_PS1_CLEAN);
        const chats = home + '/.claude/chats.db';
        fs.writeFileSync(chats, 'X'.repeat(1048576));
        fs.writeFileSync(sentinel, '1');
        ...
        // ни backup (robocopy → exit>=8), ни snapshot (Copy-Item → sharing violation) не снимут
        // консистентную копию → config обязан прерваться ДО wipe (детерминированно, без гонки).
        ...
        // warm-up: ждём, пока эксклюзивная блокировка реально захвачена (маркер ready)
        let tries = 0;
        ...
        const r = runConfigPs1Clean(home, clone);
        assert(r.status !== 0, 'ненулевой выход (консистентную копию chats.db не снять → abort): ' + (r.stdout || '') + (r.stderr || ''));
        assert(!fs.existsSync(home + '/.install-ran'), 'install.ps1 НЕ запускался (abort ДО wipe)');
        assert(!/Вернул твои ключи/.test(r.stdout || ''), 'никакого ложного «Вернул»/зелёного рапорта');
        assert(fs.existsSync(chats), 'живой chats.db на месте (не вайпнут)');
      } finally {
        try { fs.rmSync(sentinel, { force: true }); } catch (e) { /* ignore */ }
        if (writer) { try { writer.kill('SIGKILL'); } catch (e) { /* ignore */ } }
        try { fs.rmSync(base, { recursive: true, force: true }); } catch (e) { /* ignore */ }
