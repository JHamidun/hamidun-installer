...
ok('main.js: IPC launch-vscode открывает папку ~/HamidunStart (не агент-чат/URI)', () => {
  ...
  // Папка теперь открывается через де-элевированный лаунчер (P0), а не прямым spawn.
  assert(/winLaunchDeElevated\(codeExe, startDir\)/.test(h), 'папка передаётся редактору де-элевированным запуском (аналог code "<папка>")');
...
if (powershellAvailable()) {
  ...
  ['vscode.ps1', '_deelev.ps1', 'extension.ps1', 'verify.ps1', 'claude-desktop.ps1', 'chatgpt-desktop.ps1'].forEach((name) => {
...
if (bashAvailable()) {
  ['vscode.sh', 'claude-desktop.sh', 'chatgpt-desktop.sh'].forEach((name) => {
...
// (2) claude-desktop.ps1: download в Admins-only secure-cache (НЕ %TEMP%) + Authenticode-подпись
//     ДО запуска (Anthropic) + fail-closed + идемпотентность + cleanup.
ok('claude-desktop.ps1: New-HmSecureStagingDir (secure-cache, НЕ %TEMP%); Authenticode Valid+Anthropic ДО запуска; exit 120; идемпотентно; cleanup', () => {
  ...
  assert(/Get-AuthenticodeSignature/.test(s), 'проверяет Authenticode-подпись');
  assert(/\.Status -ne 'Valid'/.test(s), 'требует Status=Valid (fail-closed)');
  assert(/-notmatch \$PUBLISHER/.test(s) && /Anthropic/.test(s), 'пин издателя Anthropic');
  // ГЕЙТ ПОРЯДКА: подпись проверяется СТРОГО ДО запуска установщика.
  assert(s.indexOf('Get-AuthenticodeSignature') < s.indexOf('Start-Process -FilePath $installer'),
    'Get-AuthenticodeSignature ИДЁТ ДО Start-Process установщика (не запускаем непроверенный бинарь)');
...
// (3) chatgpt-desktop.ps1: Windows = MS Store через winget; Authenticode winget.exe (Microsoft)
//     ДО запуска; msstore; идемпотентность; fail-closed 120.
ok('chatgpt-desktop.ps1: Authenticode winget.exe (Microsoft) ДО запуска; msstore install; идемпотентно; exit 120', () => {
  const s = DT_CHATGPT_PS1();
  assert(/Get-AuthenticodeSignature/.test(s), 'проверяет подпись бинаря, который исполняет (winget.exe)');
  assert(/\.Status -ne 'Valid'/.test(s), 'требует Status=Valid (fail-closed)');
  assert(/Microsoft Corporation/.test(s) && /-notmatch \$PUBLISHER/.test(s), 'пин издателя Microsoft (winget)');
  assert(s.indexOf('Get-AuthenticodeSignature') < s.indexOf('& $winget install'),
...
// (4) claude-desktop.sh: codesign+spctl+TeamID(Anthropic) ДО установки; curl https; идемпотентно; 120.
ok('claude-desktop.sh: codesign --verify + spctl + пин TeamID Anthropic ДО ditto; curl --proto https; идемпотентно; exit 120', () => {
  const s = DT_CLAUDE_SH();
  assert(/codesign --verify --deep --strict/.test(s), 'проверяет подпись бандла');
  assert(/spctl --assess --type execute/.test(s), 'проверяет нотаризацию (Gatekeeper)');
  assert(/Q6L2SF6YDW/.test(s) && /TeamIdentifier/.test(s), 'пин Team ID Anthropic');
...
// (5) chatgpt-desktop.sh: codesign+spctl+authority(OpenAI) ДО установки; идемпотентно; 120.
ok('chatgpt-desktop.sh: codesign --verify + spctl + authority OpenAI ДО ditto; идемпотентно; exit 120', () => {
  const s = DT_CHATGPT_SH();
  assert(/codesign --verify --deep --strict/.test(s), 'проверяет подпись');
  assert(/spctl --assess --type execute/.test(s), 'проверяет нотаризацию');
  assert(/Developer ID Application: \$PUBLISHER/.test(s) && /OpenAI/.test(s), 'пин издателя OpenAI (authority)');
  assert(s.indexOf('verify_desktop_app "$APP"') < s.indexOf('ditto "$APP" "$STAGING"'),
    'подпись проверяется ДО установки (ditto)');
...
ok('grep-инвариант: нет download-в-user-writable + нет запуска бинаря без проверки подписи', () => {
  ...
  // Windows Claude: единственный запуск скачанного бинаря — Start-Process $installer, и он ПОСЛЕ подписи.
  assert(!/Start-Process -FilePath \$installer[\s\S]*Get-AuthenticodeSignature/.test(cps) ||
         cps.indexOf('Get-AuthenticodeSignature') < cps.indexOf('Start-Process -FilePath $installer'),
    'claude-desktop.ps1: установщик не запускается до Authenticode-гейта');
  // Никаких -OutFile в TEMP.
  [DT_CLAUDE_PS1()].forEach((s) => {
...
ok('P2 (main.js): launch-vscode проверяет statSync(startDir).isDirectory() — не открывает файл/несуществующее', () => {
  const s = EG_MAIN();
  const li = s.indexOf("ipcMain.handle('launch-vscode'");
  const lh = s.slice(li, li + 1200);
  assert(/statSync\(startDir\)\.isDirectory\(\)/.test(lh), 'после mkdir подтверждаем, что startDir — директория');
...
// P2 (vscode.sh): карантин снимается в ТОМ ЖЕ scoped admin_run, что и cp; таргет — один бандл.
ok('P2 (vscode.sh): xattr -dr com.apple.quarantine внутри admin_run(cp ...); строго на $APP, не рекурсивно по /Applications', () => {
  const s = EG_SH();
  assert(/admin_run "cp -R '\$SRC' '\$APP' && \/usr\/bin\/xattr -dr com\.apple\.quarantine '\$APP'"/.test(s),
    'cp и снятие карантина — одним root-scoped admin_run, таргет на $APP');
  assert(!/xattr -dr com\.apple\.quarantine "\$APP" 2>\/dev\/null \|\| true/.test(s), 'старый глушёный user-xattr убран');
});
