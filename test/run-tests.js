...
// P0 (launch): редактор стартует ДЕ-ЭЛЕВИРОВАННО, elevated-процесс НЕ spawn'ит Code.exe напрямую.
ok('P0 launch (main.js): winLaunchDeElevated — scheduled task LIMITED + explorer fallback; нет прямого spawn Code.exe', () => {
  const s = EG_MAIN();
  const at = s.indexOf('function winLaunchDeElevated(');
  assert(at !== -1, 'де-элевирующий лаунчер объявлен');
  const h = s.slice(at, at + 2400);
  assert(/New-ScheduledTaskAction/.test(h) && /RunLevel Limited/.test(h), 'primary: одноразовая scheduled task с /RL LIMITED (medium integrity)');
  assert(/LogonType Interactive/.test(h), 'задача от ТЕКУЩЕГО интерактивного пользователя');
  assert(/explorer\.exe/.test(h), 'fallback — explorer.exe (наследует medium-integrity токен shell)');
  const li = s.indexOf("ipcMain.handle('launch-vscode'");
  const lh = s.slice(li, li + 1200);
  assert(/winLaunchDeElevated\(codeExe/.test(lh), 'обработчик зовёт де-элевирующий лаунчер');
  assert(!/spawn\(codeExe/.test(lh), 'обработчик НЕ запускает Code.exe напрямую (нет elevated-spawn)');
});
...
// P0 (install): vscode.ps1 исполняет бинарь VS Code ТОЛЬКО де-элевированно + доверенный сигнал реестра.
ok('P0 install (vscode.ps1): Invoke-DeElevated (schtasks LIMITED, fail-closed) + Get-VsCodeCli (HKCU/HKLM Uninstall); нет прямого elevated code.cmd', () => {
  const s = EG_PS1();
  assert(/function Invoke-DeElevated/.test(s), 'есть де-элевирующий раннер');
  assert(/New-ScheduledTaskAction/.test(s) && /RunLevel Limited/.test(s), 'де-элевация через scheduled task /RL LIMITED (medium integrity)');
  assert(/LogonType Interactive/.test(s), 'от текущего интерактивного пользователя');
  ...
  assert(/Invoke-DeElevated \$cli @\('--install-extension'/.test(s), 'install-extension — де-элевированно');
  assert(/Invoke-DeElevated \$cli @\('--list-extensions'\)/.test(s), 'list-extensions — де-элевированно');
