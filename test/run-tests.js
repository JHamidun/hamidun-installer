...
ok('targets: nomad — config.yaml hermes НЕ в целях (в preserve); P0-3 клон гейтится НАШИМ маркером; P0-2 БЕЗ uvtool; P1-4 venv = hermes-agent', () => {
  const plan = utMod.uninstallTargets('nomad', { platform: 'win32', home: 'C:\\Users\\t' });
  targetPathsOf(plan).forEach((p) => assert(!/config\.yaml$/i.test(p), 'config.yaml не удаляется: ' + p));
  assert((plan.preserve || []).some((p) => /hermes\\config\.yaml$/i.test(p)), 'config.yaml в preserve');
  // P0-3: гейт — наш уникальный маркер, НЕ pyproject.toml (он есть у чужих проектов)
  const clone = plan.targets.find((t) => t.type === 'dirtree' && /nomad-src$/i.test(t.path));
  assert(clone && clone.onlyIfContains === '.hamidun-nomad', 'nomad-src: гейт = наш маркер .hamidun-nomad');
  assert(!plan.targets.some((t) => t.onlyIfContains === 'pyproject.toml'), 'pyproject.toml больше НЕ гейт');
  // P0-2: НИКАКОГО uvtool-экзека (user-writable uv.exe под elevated = RCE)
  assert(!plan.targets.some((t) => t.type === 'uvtool'), 'типа uvtool в плане НЕТ');
  // P1-4: venv-каталоги — точное trusted-имя пакета hermes-agent, не «nomad»
  const venvs = plan.targets.filter((t) => t.type === 'dirtree' && /[\\/]uv[\\/]tools[\\/]/i.test(t.path));
  assert(venvs.length === 2, 'две venv-цели (Roaming + .local/share): ' + JSON.stringify(venvs.map((v) => v.path)));
  venvs.forEach((v) => assert(/[\\/]hermes-agent$/i.test(v.path), 'venv = tools/hermes-agent: ' + v.path));
  assert(!targetPathsOf(plan).some((p) => /[\\/]uv[\\/]tools[\\/]nomad$/i.test(p)), 'чужой tools/nomad НЕ в целях');
  // P1-4: кастомное имя из ctx проходит гигиену; мусор → дефолт hermes-agent
  const evil = utMod.uninstallTargets('nomad', { platform: 'win32', home: 'C:\\Users\\t', nomadTool: '..\\..\\evil' });
  evil.targets.filter((t) => t.type === 'dirtree' && /[\\/]uv[\\/]tools[\\/]/i.test(t.path))
    .forEach((v) => assert(/[\\/]hermes-agent$/i.test(v.path), 'грязное имя тула → дефолт hermes-agent: ' + v.path));
});
...
ok('PRESERVE: nomad — hermes config.yaml ЦЕЛ, SOUL.md/скин удалены; P0-3 nomad-src БЕЗ нашего маркера НЕ тронут (даже с pyproject.toml)', () => {
  const home = mkHomeDir();
  try {
    const plan = utMod.uninstallTargets('nomad', { platform: 'win32', home });
    const hh = path.join(home, 'AppData', 'Local', 'hermes');
    fs.mkdirSync(path.join(hh, 'skins'), { recursive: true });
    fs.writeFileSync(path.join(hh, 'SOUL.md'), 'soul');
    fs.writeFileSync(path.join(hh, 'skins', 'nomad.yaml'), 'skin');
    fs.writeFileSync(path.join(hh, 'config.yaml'), 'api_key: USER-KEY');
    // P0-3: ЧУЖОЙ Python-проект в этом месте — С pyproject.toml, но БЕЗ нашего
    // маркера .hamidun-nomad. Раньше сносился; теперь fail-closed НЕ трогаем.
    const src = path.join(home, 'AppData', 'Local', 'nomad-src');
    ...
    assert(!fs.existsSync(path.join(hh, 'skins', 'nomad.yaml')), 'скин удалён');
    assert.strictEqual(fs.readFileSync(path.join(hh, 'config.yaml'), 'utf8'), 'api_key: USER-KEY', 'config.yaml ЦЕЛ');
    assert(fs.existsSync(path.join(src, 'pyproject.toml')) && fs.existsSync(path.join(src, 'user-notes.txt')),
      'ЧУЖОЙ nomad-src (pyproject.toml без нашего маркера) ПОЛНОСТЬЮ ЦЕЛ');
    assert(!fs.existsSync(venv), 'venv uv-тула hermes-agent удалён');
  } finally { dropDir(home); }
});

ok('P0-3: nomad-src С нашим маркером .hamidun-nomad → удаляется; postconditions это подтверждают', () => {
  const home = mkHomeDir();
  try {
    const plan = utMod.uninstallTargets('nomad', { platform: 'win32', home });
    const src = path.join(home, 'AppData', 'Local', 'nomad-src');
    fs.mkdirSync(src, { recursive: true });
    fs.writeFileSync(path.join(src, 'pyproject.toml'), '[project]\nname = "hermes-agent"');
    fs.writeFileSync(path.join(src, '.hamidun-nomad'), 'installed-by: hamidun-setup');
    const results = execFsPlan(plan, home);
    assert(results.every((x) => x.r.status !== 'failed'), 'без отказов: ' + JSON.stringify(results.map((x) => x.r)));
    assert(!fs.existsSync(src), 'НАШ клон (с маркером) удалён');
...
ok('P0-2 (source): деинсталлятор НЕ запускает user-writable uv (нет uvtool/uv tool uninstall/findUvBinary в КОДЕ)', () => {
  ...
  // Инвентарь uv-тула удаляется точными dirtree-целями с trusted-именем (tool).
  assert(/'tools', tool\)/.test(ut), 'venv uv-тула удаляется dirtree-целью (…/tools/<tool>)');
});
