...
(function preflightGateTests() {
  ...
  // Проба зеркал обязана ОТЛИЧАТЬ «сервер не ответил» от «сервер ответил отказом».
  //
  // До 31.08 проба была одна, без повтора: восемь штук разом к двум хостам с
  // таймаутом 8 с. Любая заминка CDN читалась как «зеркало мёртво», а при обоих
  // мёртвых гейт печатал СТОП с советом перезалить гигабайты. Замер: три прогона
  // подряд на одной машине дали 7, 3 и 0 «мёртвых» зеркал — измеритель мерил сеть,
  // а не зеркала. После повтора — 0, 0, 0.
  //
  // Проверяем ИСТОЧНИК, а не поведение: настоящая проба ходит в интернет, и тест на
  // ней сам стал бы тем, что мы чиним (см. Н.3 — зависимость теста от сети).
  ok('release-check: не ответившее зеркало переспрашивается, ответивший отказом — нет', () => {
    const src = fs.readFileSync(path.join(ROOT, 'tools', 'release-check.js'), 'utf8');
    assert(/const answered = \(r\) => !!r && typeof r\.code === 'number' && r\.code > 0;/.test(src),
      'есть предикат «сервер ответил» по наличию настоящего HTTP-кода');
    assert(/if \(answered\(res\[k\]\)\) continue;/.test(src),
      'ответивший отказом (404/403) НЕ переспрашивается — это факт, а не помеха');
    assert(/res\[k\] = await remoteFetch\.probeMirror\(slice\[k\]\.m\.url, 15000\);/.test(src),
      'не ответившее зеркало переспрашивается с бОльшим таймаутом');
    assert(/не ответил дважды/.test(src),
      'в отчёте сказано, что попыток было ДВЕ — иначе «нет ответа» снова читается как приговор');
    // Отрицательный контроль на смысл: одиночная проба без повтора вернулась бы —
    // и текст «нет ответа» вместе с ней.
    assert(!/\(r\.code \? ` \(HTTP \$\{r\.code\}\)` : ' \(нет ответа\)'\)/.test(src),
      'старая формулировка «нет ответа» (после ОДНОЙ попытки) не вернулась');
  });
})();
