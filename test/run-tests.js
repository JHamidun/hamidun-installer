...
ok('P0-1 (scripts): nomad.sh/ps1 — skip → exit 120 (не 0), real-fail → exit 1; маркеры .hamidun-nomad НЕ пишутся', () => {
  ...
  assert(/"\$CLONE_ATTEMPTED" = "1"[\s\S]{0,400}exit 1/.test(nsh), 'nomad.sh: пытались клонировать + нет источника → exit 1');
  ...
  assert(/\$cloneAttempted -and[\s\S]{0,400}exit 1/.test(nps), 'nomad.ps1: пытались клонировать + нет источника → exit 1');
