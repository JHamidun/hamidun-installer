...
ok('targets: bridge — config.json (SSH-креды) НИ в одной цели, зато в preserve; reg — точное HKCU\\...\\Run', () => {
  for (const plat of ['win32', 'darwin']) {
    ...
    if (plat === 'win32') {
      ...
      assert(pls.length === 2 && pls.every((t) => t.marker === '# Hamidun Bridge CLI proxy'), 'profileline: точный маркер');
      assert(pls.every((t) => /[\\/](\.zshrc|\.bash_profile)$/.test(t.file)), 'profileline: только разрешённые rc-файлы');
    }
  }
});

ok('targets: nomad — config.yaml hermes НЕ в целях (в preserve); клон исходников гейтится pyproject.toml', () => {
  ...
  assert(clone && clone.onlyIfContains === 'pyproject.toml', 'nomad-src: санити-гейт pyproject.toml');
  assert(plan.targets.some((t) => t.type === 'uvtool' && t.tool === 'nomad'), 'uv tool uninstall в плане');
});
...
ok('removeProfileLine: rc вне списка (crafted ~/.claude/settings.json как profileline) → ЗАЩИТА; из ~/.zshrc уходят ТОЛЬКО строки-маркеры', () => {
  const home = mkHomeDir();
  try {
    const opts = { home, platform: process.platform };
    fs.mkdirSync(path.join(home, '.claude'), { recursive: true });
    fs.writeFileSync(path.join(home, '.claude', 'settings.json'), '{"hooks":"# Hamidun Bridge CLI proxy"}');
    const r1 = uxMod.removeProfileLine(path.join(home, '.claude', 'settings.json'), '# Hamidun Bridge CLI proxy', opts);
    ...
    assert.strictEqual(uxMod.removeProfileLine(path.join(home, 'notes.txt'), '# Hamidun Bridge CLI proxy', opts).status,
      'failed', 'не-rc файл → отказ');
    // легитимный ~/.zshrc: только строки с маркером
    fs.writeFileSync(path.join(home, '.zshrc'),
      'export A=1\n. proxy.env # Hamidun Bridge CLI proxy\nalias ll="ls -la"\n');
    const r2 = uxMod.removeProfileLine(path.join(home, '.zshrc'), '# Hamidun Bridge CLI proxy', opts);
...
function execFsPlan(plan, home) {
  ...
  for (const t of plan.targets) {
    ...
    } else if (t.type === 'profileline') r = uxMod.removeProfileLine(t.file, t.marker, opts);
    results.push({ t, r });
  }
  return results;
}
...
ok('PRESERVE: nomad — hermes config.yaml ЦЕЛ, SOUL.md/скин удалены; nomad-src без pyproject НЕ тронут', () => {
  const home = mkHomeDir();
  try {
    ...
    // «клон» БЕЗ pyproject.toml (чужая папка с тем же именем) — не трогаем
    ...
    assert(fs.existsSync(path.join(src, 'user-notes.txt')), 'nomad-src без pyproject.toml НЕ тронут (санити-гейт)');
  } finally { dropDir(home); }
});
...
ok('main.js (source): reg — только HKCU из аллоулиста ключей; pathentry сохраняет тип REG_(EXPAND_)SZ и не раскрывает %VAR%', () => {
  ...
  assert(/REG_\(\?:EXPAND_\)\?SZ/.test(s.replace(/\\/g, '')) || s.indexOf('(REG_(?:EXPAND_)?SZ)') !== -1,
    'тип значения PATH читается и сохраняется');
