...
async function clickInstall(page) {
  ...
  try {
    ...
    check('интерфейс ответил на клик «Установить»', true);
    return;
  } catch (e) {
    // ПРОВАЛЕННАЯ ПРОВЕРКА, а не предупреждение. Раньше здесь стоял только console.log,
    // и прогон оставался зелёным: CI читает КОД ВОЗВРАТА, а он зависит только от
    // счётчика проваленных проверок. То есть возврат «окно не отвечает» — ровно тот
    // дефект, ради которого всё это чинилось, — был бы проглочен молча. Синхронные
    // блокировки главного процесса в коде ещё остались (winFileAdminOwned и др.),
    // так что регрессия достижима.
    check('интерфейс ответил на клик «Установить»', false,
      'подтверждение не пришло за 15 с — главный процесс занят синхронной работой, ' +
      'окно в этот момент не отвечает: ' + (e && e.message || e));
  }
  // Дожимаем событием НЕ чтобы скрыть провал (он уже засчитан), а чтобы прогон дошёл
  // до конца и дал полную картину, а не обрывался на первом шаге.
