...
3. **OV/EV код-сертификат от CA** (DigiCert/Sectigo/Certum, ~$200–600/год) — EV даёт мгновенную
   репутацию SmartScreen, но требует аппаратный токен, облачный HSM или приложение вроде Certum
   SimplySign. Дороже и муторнее Azure.

   **Файла `.pfx` для таких сертификатов больше не выдают** (проверено 06.09.2026). По требованиям
   CA/Browser Forum приватный ключ обязан жить в защищённом крипто-модуле, и экспорт в PFX закрыт.
   Значит вариант «положить base64 .pfx в секреты GitHub» для НОВОГО сертификата невозможен —
   подписывать придётся там, где ключ доступен: локально через виртуальную смарт-карту SimplySign
   либо через облачный подписыватель с API. Плюс с 27.02.2026 максимальный срок сертификата —
   459 дней вместо трёх лет, то есть перевыпуск становится регулярной задачей.

   Именно поэтому вариант 2 (Azure) стоит выше: там ключ в облачном HSM Microsoft, и подпись идёт
   прямо из CI без токенов и ручных шагов.
