...
function main(argv) {
  ...
  if (args.includes('--json')) {
    ...
    printPreflight(rep);
    // Связность пака. Свежесть пака гейт уже проверяет, но «пак свежий» и «пак
    // работает» — разные вещи: на машине ученика конфиг разложился успешно, а
    // плагины были включены без маркетплейса, правила ссылались на отсутствующие
    // скрипты, MCP объявлены с путями в чужой домашний каталог. Всё это
    // «успешно установилось», потому что проверяли раскладку файлов, а не работу.
    //
    // СТОПы БЛОКИРУЮТ сборку. Здесь раньше стояло обратное, и обоснование звучало
    // так: «audit-pack проверяет, поднялись ли MCP-серверы, а на раннере они
    // заведомо не подняты, — гейт, красный по причине окружения, отключат через
    // неделю». Довод верный по форме и неверный по факту: `audit-pack.js` НЕ
    // ЗАПУСКАЕТ ни одного процесса и не ходит в сеть (проверено обходом файла:
    // ни child_process, ни net, ни fetch). Он читает `settings.json`, правила и
    // каталог пака и сверяет пути на диске. Значит все его СТОПы детерминированы
    // содержимым пака: на сборочном раннере и на машине ученика ответ один и тот
    // же. Единственный документ, который мешал включить единственную ФУНКЦИОНАЛЬНУЮ
    // проверку продукта, сам противоречил коду — тот самый класс расхождения, ради
    // которого написан ADR-008.
    //
    // Замечания (`~`) блокирующими НЕ становятся: там счётчики и «N из M MCP через
    // npx» — это про окружение пользователя, а не про содержимое пака.
    const annotate = (msg, kind) => {
      if (process.env.GITHUB_ACTIONS === 'true') {
        console.log('::' + (kind || 'warning') + ' title=Связность конфиг-пака::' + msg);
      }
    };
    let packBroken = 0;
    try {
      ...
      if (stops.length) {
        packBroken = stops.length;
        annotate('конфиг-пак разложится, но работать будет не весь: ' + stops.length +
          ' проблем(ы). Первая: ' + stops[0].replace(/^СТОП\s*/, ''), 'error');
        console.log('[preflight] СБОРКА ОСТАНОВЛЕНА: пак доедет формально целым и ' +
          'функционально мёртвым. Почини перечисленное либо запусти с --allow-broken-pack, ' +
          'если это осознанно.');
      } else {
        // Ненулевой код без единого СТОПа — сам аудит сломался (нет пака, упал на
        // разборе). Это не повод рушить сборку, но и молчать нельзя.
        console.log('[preflight] audit-pack завершился с ошибкой, но СТОПов не назвал — ' +
          'проверка связности НЕ выполнена.');
        annotate('проверка связности пака не выполнена — audit-pack упал без вердикта');
      }
    }
    if (packBroken && !args.includes('--allow-broken-pack')) return 1;
  }
