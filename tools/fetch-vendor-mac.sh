...
# Качаем во ВРЕМЕННЫЙ файл и переносим на место только при успехе — зеркало Dl() из
# fetch-vendor.ps1, там же полный разбор. Коротко: curl без --remove-on-error оставляет
# на диске частично скачанное, следующий прогон пропускает обрубок по `[ -f "$2" ]` как
# готовый, а сумма для манифеста целостности снимается С НЕГО ЖЕ — и дальше всё сходится
# на каждом гейте, потому что сверяют обрубок сам с собой. Здесь единственное место, где
# это можно разорвать: под настоящим именем на диск попадает только докачанное до конца.
# _lib.sh-версия dl() делает `rm -f` при провале, но эта её переопределяет — значит
# защиту надо повторить, а не унаследовать.
dl() {
  if [ -f "$2" ]; then echo "  skip $(basename "$2")"; return; fi
  echo "  GET $1"
  rm -f "$2.part" 2>/dev/null || true
  if curl -fsSL "$1" -o "$2.part" && [ -s "$2.part" ]; then
    mv -f "$2.part" "$2"
  else
    echo "  ! не скачалось $(basename "$2")"
    rm -f "$2.part" 2>/dev/null || true
  fi
}
...
nomad_src_sanitize() {
  ...
  for rel in .superpowers .plans .github .claude \
             docs \
             deploy scripts/tests; do
    if [ -e "$S/$rel" ]; then rm -rf "$S/$rel"; echo "    - удалил $rel/"; fi
  done
  # 1a) СТОРОЖ вместо одного лишь списка — зеркало fetch-vendor.ps1, там же
  #     разобрано почему. Коротко: поимённый список отстаёт МОЛЧА. Он писался
  #     под структуру Nomad на тот день, за 422 коммита завелись .plans/,
  #     .github/, docs/nomad-upgrade-reports/ — и все уехали покупателям, потому
  #     что строки для них никто не дописал. Признак устойчивее имени.
  for d in "$S"/.*; do
    b=$(basename "$d")
    case "$b" in
      .|..|.python-version) continue ;;
    esac
    if [ -d "$d" ]; then rm -rf "$d"; echo "    - удалил по признаку (точка в начале): $b/"; fi
  done
  #     Тот же сторож — по ФАЙЛАМ верхнего уровня. Прежняя редакция смотрела только
  #     каталоги (-type d), и внутренние документы, лежащие файлами, проходили мимо:
  ...
  while IFS= read -r f; do
    [ -f "$f" ] || continue
    rm -f "$f"; echo "    - удалил по признаку (внутренний документ): $(basename "$f")"
  done <<EOF
$(find "$S" -maxdepth 1 -type f \
    \( -iname '*_plan*' -o -iname '*plan_*' -o -iname '*_spec*' -o -iname '*spec_*' \
       -o -iname '*internal*' -o -iname '*audit*' -o -iname '*backlog*' -o -iname 'phase[0-9]*' \) \
    2>/dev/null)
EOF
...
if grep -q '"nomad"' "$ROOT/components.json" 2>/dev/null; then
  ...
  # ПИН релизной версии агента: дефолт = стабильный тег, НЕ движущийся main (иначе в
  # раздачу утечёт незакоммиченная/недоделанная работа при активной разработке агента).
  # Бамп версии = сменить дефолт здесь (и в fetch-vendor.ps1) или задать env HM_NOMAD_REF.
  # 05.09.2026: поднят с тега v2026.7.22 (0.17.0) на КОММИТ 0.18.1 — держать
  # синхронно с tools/fetch-vendor.ps1, там же разобрано, почему коммит, а не
  # тег: свежих тегов НАШЕЙ линии нет, все v2026.8.* принадлежат апстриму
  # NousResearch/hermes-agent и нашему main не предки.
  NOMAD_REF="${HM_NOMAD_REF:-5a73e548233d2f830e3f0d619509012394a02280}"
