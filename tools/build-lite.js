...
function assertRegistryMatchesChecksums(platform) {
  ...
  for (const e of (reg.components || [])) {
    ...
    for (const [name, sha] of Object.entries(gated)) {
      ...
      if (!local) {
        // Прежний комментарий здесь утверждал «имени нет в манифесте → рантайм-гейт его
        // не проверяет», и запись молча пропускалась. Это неверно ровно наоборот:
        // Confirm-HmArtifact (_verify.ps1:82-86) ищет запись ПО ИМЕНИ и при её отсутствии
        // печатает «в checksums.json нет записи — возможна подмена» и делает exit 1.
        //
        // Само расхождение означает, что манифест изменился ПОСЛЕ публикации компонента:
        // gatedFiles собирается push-component'ом только из имён, которые были в
        // манифесте на тот момент (push-component.py:107-108). То есть архив в S3 несёт
        // файл, который вшитый манифест больше не описывает, — сборка при этом зелёная,
        // а установка падает у КАЖДОГО lite-пользователя после честной загрузки.
        throw new Error(
          `вшитый checksums.json не описывает файл «${name}» из опубликованного «${e.remoteId}»: ` +
          `рантайм-гейт ищет запись по имени и при её отсутствии останавливает установку ` +
          `(«возможна подмена»). Значит манифест разошёлся с тем, что лежит в хранилище. ` +
