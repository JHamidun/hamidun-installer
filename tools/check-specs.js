...
function main() {
  ...
  for (const f of features) {
    ...
    if (!testLine) add(f.id, 'нет шапки', 'строка «- **Тесты:** …» отсутствует');
      ...
      // Диапазон строк отрезаем так же, как у путей в «Код»: спеки пишут
      // `test/e2e-gui.js:374-398`, и без этого проверка искала файл с двоеточием и
      // номерами в имени — то есть падала на совершенно верной ссылке. Ошибка моя,
      // не спеки: у кодовых путей суффикс срезался, у тестовых нет.
      const files = backticked(testLine)
        .map((p) => p.split(':')[0])
